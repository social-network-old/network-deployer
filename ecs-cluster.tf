...
resource "aws_iam_role" "ecs-cluster-runner" {
  name               = "${var.app_name}-cluster-runner"
  assume_role_policy = data.aws_iam_policy_document.instance-assume-role.json
}

data "aws_caller_identity" "current" {}

data "aws_iam_policy_document" "ecs-cluster-runner" {
  statement {
...
resource "aws_iam_role_policy" "ecs-cluster-runner" {
  name   = "${var.app_name}-cluster-runner-policy"
  role   = "${aws_iam_role.ecs-cluster-runner.name}"
  policy = data.aws_iam_policy_document.ecs-cluster-runner.json
}

resource "aws_iam_instance_profile" "ecs-cluster-runner" {
...
resource "aws_instance" "ecs-cluster-runner" {
  ...
  iam_instance_profile        = "${aws_iam_instance_profile.ecs-cluster-runner.name}"
